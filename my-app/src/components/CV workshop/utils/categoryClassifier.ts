...
export type ExperienceCategory = 'IT & Software Engineering' | 'General Labor & Operations';
...
/**
 * Classifies candidate skills into Technical, Labor, or Personal categories via Qwen.
 */
...
Return STRICT JSON format:
{
  ...
  try {
    ...
    return {
      technical: [...technical, ...missingSkills],
      labor,
      personal,
    };
  } catch (error) {
    console.warn('[LLMCategorizer] Skill classification fallback engaged:', error);
...
): Promise<Record<string, ExperienceCategory>> {
...
Return STRICT JSON mapping 'id' to 'category':
{
  ...
  try {
    ...
    return cleanAndParseLLMJson<Record<string, ExperienceCategory>>(response);
  } catch (error) {
    console.warn('[LLMCategorizer] Experience classification fallback engaged:', error);
    return {};
  }
}